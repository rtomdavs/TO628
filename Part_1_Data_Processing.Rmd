...
Issue 1: for categorical features with more than 2 levels, some levels might have few rows of data, while others have a lot. If we convert levels with few data to dummies, the dummies would only add noise when training model.
Our solution: for each categorical feature with this problem, we will group the trivial levels to create an aggregated dummy variable called "Others". We define any levels with less than 5% of total values, or top 3 levels with most values (if these is only one levels with more than 5%), as trivial. 

```{r}
library(dplyr)

# Marital Status
nonenr %>% count(Marital.status) %>% mutate(n = n/sum(n))

# Application Mode
nonenr %>% count(Application.mode) %>% mutate(n = n/sum(n))

# Course
nonenr %>% count(Course) %>% mutate(n = n/sum(n))

# Previous Qualification
nonenr %>% count(Previous.qualification) %>% mutate(n = n/sum(n))

# Nacionality
nonenr %>% count(Nacionality) %>% mutate(n = n/sum(n))

# Mother's Qualification
nonenr %>% count(Mother.s.qualification) %>% mutate(n = n/sum(n))

# Father's Qualification
nonenr %>% count(Father.s.qualification) %>% mutate(n = n/sum(n))

# Mother's Occupation
nonenr %>% count(Mother.s.occupation) %>% mutate(n = n/sum(n))

# Father's Occupation
nonenr %>% count(Father.s.occupation) %>% mutate(n = n/sum(n))
```
...
Issue 2: we have observed that `model.matrix()` creates a dummy for all N levels of the categorical feature `Marital Status`, while we only need N-1 levels of dummies to avoid the problem of multicollinearity. 
Our solution: remove a level for these categorical features manually 
...
```{r}
# Feature Engineering: Apply the same transformations on the prediction dataset `enr`
...
```{r}
# training set
write.csv(drop_norm, file = "cleaned_nonenrolls.csv")
# prediction set
write.csv(enroll_norm, file = "cleaned_enrolls.csv")
```
